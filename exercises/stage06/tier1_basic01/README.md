...
- `is_valid_amount(amount) -> bool` (`@staticmethod`) -- `True` if `amount` is an `int`/`float` (and not a `bool`) and `amount >= 0`. It's a `@staticmethod` because it doesn't need `self` at all -- it's just a validation helper that happens to live on the class.
