...
- `categorize_sales(df) -> pd.DataFrame` -- return a copy with a new `"tier"` column via `df["sales"].apply(_categorize_amount)` (already given), which returns `"low"` (`x < 100`), `"medium"` (`100 <= x < 300`), or `"high"` (`x >= 300`).
