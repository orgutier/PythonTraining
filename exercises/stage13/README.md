...
Each exercise below lives in its own folder with its own `solution.py` (the entry point) and `README.md` (the problem statement), and is independently testable with `python tools/cli.py test stage13_<name>` (e.g. `python tools/cli.py test stage13_exercise01`). Work through them in order.
