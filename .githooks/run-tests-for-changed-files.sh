...
# exercises/stageNN/exerciseXX/*.py (including solution.py itself and any
# helper submodule next to it), exams/examNN/solution.py, or
# challenges/challengeNN/solution.py to its test id, and runs "python
# tools/cli.py test <id>" once per unique id found -- not the full suite,
# not the whole stage it lives in, just the exercise/exam/challenge that
# actually changed. Exits non-zero if any of those runs fail; exits 0
# (nothing to test) if no matching file appears in the input at all.
