...
# exercises/weekNN/solution.py or challenges/challengeNN/solution.py to
# its test id, and runs "python tools/cli.py test <id>" once per unique
# id found -- not the full suite. Exits non-zero if any of those runs
# fail; exits 0 (nothing to test) if no matching solution.py appears in
# the input at all.
...
if [ -z "$ids" ]; then
    echo "No exercises/weekNN/solution.py or challenges/challengeNN/solution.py changed -- nothing to test."
    exit 0
fi
